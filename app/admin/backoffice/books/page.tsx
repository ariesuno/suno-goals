...
export default function BooksPage() {
  ...
  const filteredBooks = books.filter(book => {
    ...
    if (filters.is_active !== undefined && book.is_active !== filters.is_active) {
      return false;
    }

    // Filtro por metas faltando
    if (filters.has_missing_goals && book.indicators_with_missing_goals === 0) {
      return false;
    }
  ...
  const booksWithAlerts = books.filter(b => b.indicators_with_missing_goals > 0 || (b.performance_level === 'critical')).length;
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => {
            ...
            setFilters({
              ...filters,
              performance_level: hasFilter ? [] : ['critical']
            });
          }}
          className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
